...
- (void)setupTouchControls {
    ...
    TouchButton *aButton = [[TouchButton alloc] initWithFrame:CGRectMake(
        screenWidth - margin - largeButtonSize - 40,
        screenHeight - margin - largeButtonSize - 100,
    ...
    // B Button - BOMB (smaller size, moved more inward)
    CGFloat smallButtonSize = 55;  // Made smaller
    TouchButton *bButton = [[TouchButton alloc] initWithFrame:CGRectMake(
        screenWidth - margin - smallButtonSize - 70,
        screenHeight - margin - smallButtonSize - 20,
    ...
    // Calculate A button center position
    CGFloat aButtonCenterX = screenWidth - margin - largeButtonSize - 40 + (largeButtonSize / 2);
    CGFloat aButtonCenterY = screenHeight - margin - largeButtonSize - 100 + (largeButtonSize / 2);
    ...
    cLeft.onPress = ^{ iOS_SetButton(IOS_BUTTON_Y, true); };
    cLeft.onRelease = ^{ iOS_SetButton(IOS_BUTTON_Y, false); };
    ...
    cDown.onPress = ^{ iOS_SetButton(IOS_BUTTON_B, true); };
    cDown.onRelease = ^{ iOS_SetButton(IOS_BUTTON_B, false); };
...
- (void)setupMotionControls {
    ...
    self.motionUpdateTimer = [NSTimer scheduledTimerWithTimeInterval:1.0/60.0
        ...
        // Barrel roll detection using Z axis (roll)
        // Threshold: 1.5 rad/s for deliberate roll input
        double rollThreshold = 1.5;
        if (rawZ < -rollThreshold) {
            // Roll left -> Z button
            iOS_SetButton(IOS_BUTTON_LEFTSHOULDER, true);
            iOS_SetButton(IOS_BUTTON_RIGHTSHOULDER, false);
        } else if (rawZ > rollThreshold) {
            // Roll right -> R button
        ...
        // Determine roll button status for debug
        NSString *rollStatus = @"---";
        if (rawZ < -rollThreshold) {
            rollStatus = @"Z (LEFT)";
        } else if (rawZ > rollThreshold) {
